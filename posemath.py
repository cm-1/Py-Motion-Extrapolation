...
def non_collinear_features(X: np.ndarray, threshold: float = 0.99):
    ...
    Parameters:
    X (np.ndarray): Input 2D array (samples x features).
    threshold (float): Correlation threshold for collinearity. Columns with correlation 
                       above this value will be removed.

    Returns:
    np.ndarray: Boolean mask indicating which columns to keep.
    np.ndarray: Upper triangular covariance matrix.
    """
    ...
    to_remove = np.any(upper_tri > threshold, axis=0)
    
    return ~to_remove, upper_tri
