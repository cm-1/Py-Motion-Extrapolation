...
jav_lstm_logger = UnscaledDistanceLogger(
    train_jav_in[..., -3:].reshape(-1, WIN_SIZE*3), train_jav_out[:, -3:], jav_scalers_3[jav_lstm_skip]
)
...
jav_lstm_hist = jav_lstm_model.fit(
    train_jav_in[..., -3:].reshape(-1, WIN_SIZE*3), train_jav_out[:, -3:],
    epochs=64, callbacks=[jav_lstm_logger]
)
...
jav_lstm_test_pred = jav_lstm_model.predict(test_jav_in[..., -3:].reshape(-1, WIN_SIZE * 3))
...
plt.plot(jav_lstm_logger.mean_distances[-64:])
